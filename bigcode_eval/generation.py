...
from bigcode_eval.utils import OpenAICompletionDataset, TokenizedDataset, complete_code, complete_code_api
...
def parallel_generations(
    ...
    from bigcode_eval.utils import OpenAICompletionDataset, TokenizedDataset, complete_code, complete_code_api, is_openai_api
    ...
    is_openai = is_openai_api(args.model)
    if is_openai:
        ds = OpenAICompletionDataset(
            task,
            dataset,
            max_length=args.max_length_generation,
        ...
        complete_function = complete_code_api
        ...
        complete_function = complete_code
    ...
    if not is_openai:
        if args.max_memory_per_gpu is not None:
            ...
            model = model.to(accelerator.device) if model is not None else None
            ds_loader = accelerator.prepare(ds_loader)
        else:
            # model.to() is not supported for 8bit and 4bit models
            model, ds_loader = accelerator.prepare(model, ds_loader)

    generations = complete_function(
        task,
        accelerator,
        model,
        tokenizer,
