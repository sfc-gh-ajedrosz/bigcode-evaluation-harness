...
from bigcode_eval.utils import is_openai_api

def optional_int(value: str) -> Optional[int]:
    """
    This function allows us to parse command-line
    args of type Optional[int]. Examples:
        --arg 1 -> 1
        --arg -10 -> -10
        --arg NoNe -> None
        --arg NULL -> None
        --arg abc -> "invalid Optional[int] value: 'abc'"
    """
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"invalid Optional[int] value: '{value}'")


def load_tokenizer(args) -> PreTrainedTokenizer:
    API_MODEL = is_openai_api(args.model)

    # args.model_args is converted from a string to a dictionary
    # in the top-level script, but we want this function to be
    # usable regardless of whether this has happened
    if isinstance(args.model_args, dict):
        model_args = args.model_args
    else:
        model_args = {k: v for k, v in re.findall('([^=]+)="?([^",]+)"?,?', args.model_args)}

    if args.left_padding:
        # left padding is required for some models like chatglm3-6b
        tokenizer = AutoTokenizer.from_pretrained(
            args.model if not API_MODEL else model_args['tokenizer'],
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum number of tokens that can be generated by the model (excluding prompt). "
            "You must make sure that --max_new_tokens plus prompt length is <= context window "
            "for all inputs."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Comma separated string arguments for model",
    )
...
def main():
    ...
    if args.load_generations_path:
        ...
        API_MODEL = is_openai_api(args.model)
        if API_MODEL:
            args.model_args = {k: v for k, v in re.findall('([^=]+)="?([^",]+)"?,?', args.model_args)}
            model = None
